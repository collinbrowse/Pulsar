...
final class AuthenticationService {
    ...
    func refreshTokenIfNeeded(appState: AppState? = nil) async throws { // swiftlint:disable:this function_body_length
        ...
        guard let refreshToken = session.refreshToken, !refreshToken.isEmpty else {
            logger.error("❌ Session expired but no refresh token available")
            if let appState {
        ...
        do {
            let newSession = try await supabaseClient.refreshSession(refreshToken: refreshToken)
            ...
            if case .httpError(let statusCode, let message) = networkError {
                if statusCode == 400,
                   let msg = message,
