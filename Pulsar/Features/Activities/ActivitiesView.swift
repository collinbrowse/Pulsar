        ...
        List {
            ForEach(activities) { activity in
                NavigationLink(value: activity) {
                    ListedActivityRow(activity: activity)
                }
            }
    ...
    var body: some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 12) {
                    Label(formatDistance(activity.distanceMeters), systemImage: "location.fill")
                    Label(formatDuration(Double(activity.elapsedTimeSeconds)), systemImage: "clock.fill")
                }
                .font(.caption)
