        ...
        ScrollView {
            VStack(spacing: 24) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                    ActivityDetailStatCard(title: "Distance", value: formatDistance(activity.distanceMeters), icon: "location.fill")
                    ActivityDetailStatCard(title: "Duration", value: formatDuration(Double(activity.elapsedTimeSeconds)), icon: "clock.fill")
                    
                    if let avgSpeed = activity.avgSpeedMps {
                        ActivityDetailStatCard(title: "Avg Speed", value: formatSpeed(avgSpeed), icon: "speedometer")
                    }
                    
                    if let elevation = activity.elevationGainMeters {
                        ActivityDetailStatCard(title: "Elevation", value: formatElevation(elevation), icon: "arrow.up.right")
                    }
                    
                    if let avgHR = activity.avgHeartRate {
                        ActivityDetailStatCard(title: "Avg HR", value: "\(avgHR) bpm", icon: "heart.fill")
                    }
                    
                    if let avgPower = activity.avgPower {
                        ActivityDetailStatCard(title: "Avg Power", value: "\(Int(avgPower))w", icon: "bolt.fill")
                    }
                }
                .padding(.horizontal)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    DetailRow(label: "Source", value: activity.sourceFile == nil ? "This device" : "File import")
                    if let fileName = activity.sourceFile {
                        DetailRow(label: "File", value: fileName)
                    }
                    DetailRow(label: "Type", value: activity.type.displayName)
                    DetailRow(label: "Privacy", value: activity.visibility.displayName)
                }
